...
- Скрипт установки — обычное копирование; просмотрите его и конфиги перед запуском
- Конфиги хранятся без секретов: пароль OBS-WebSocket, пустые/дефолтные кэши и служебные дефолты исключены (см. историю коммита «чистка»)
- Пароль OBS-WebSocket, если использовался, лучше сменить — ранее попал в публичную историю репозитория
